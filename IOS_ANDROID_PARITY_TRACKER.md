...
## Parity updates from the 2026-07-04 iOS working-tree audit

iOS source reviewed: current dirty-tree Songbook/Cantus changes in
`CantusCatalog.swift`, `CantusService.swift`, `CantusImportObserver.swift`,
`CantusImportView.swift`, `ChordProParser.swift`, `Song.swift`,
`SongbookObserver.swift`, `SongbookView.swift`, `SongDetailView.swift`, and
`SongEditorView.swift`.

| ID | Status | iOS source / behavior | Android evidence | Required Android result | Verification |
| --- | --- | --- | --- | --- | --- |
| SONG-004 | VERIFY | iOS adds an authenticated Cantus catalog import flow, canonical `chordedLyrics` storage, `cantusSlug`, PPTX links, typed `media.audioFiles` Voice Kits, Browse Song Catalog as the primary add action, Add Manually as fallback, catalog search/filter/selection UI, duplicate Added badges, and ChordPro parsing/serialization for `{Section}` plus `[G]inline` chords. | Android now adds `CantusService`, `CantusCatalog`, `CantusImportViewModel`, and `CantusImportScreen`; routes `songbook/cantus-import` before dynamic song detail; changes the Songbook plus menu to Browse Song Catalog/Add Manually; persists `chordedLyrics`, `chordedLyricsUpdatedAt`, `cantusSlug`, `pptxLink`, and labeled Cantus `audioFiles`; opens PPTX from Song detail; exposes PPTX in the editor; supports the shared `contralto` voice type; and moves the shared ChordPro/parser contract into `data/songbook`. | Live-check the backend `/cantus/*` proxy with a signed-in account, compare Android catalog layout/filters/import bar against iOS, and confirm imported PDF/PPTX/multiple audio/chords render from a real Cantus song. | Focused `ChordProParserTest`, `MediaSongTest`, `CantusCatalogTest`, `SongbookViewModelTest`, full `:app:testDebugUnitTest`, `:app:compileDebugKotlin`, `:app:lintDebug`, `:app:assembleDebug`, and `git diff --check` passed 2026-07-04. Live device/proxy verification remains. |
...
| ID | Status | iOS source / behavior | Android evidence | Required Android result | Verification |
| --- | --- | --- | --- | --- | --- |
| TRANSPORT-005 | DONE | iOS `VehicleMutation`, `VehicleService`, `VehicleObserver`, `MyTransportationView`, `CampingVehiclesView`, and Firestore rules now require `offeredSeatCount > 0` before new add/request/approve/invite passenger actions. | Android mirrors the guard in `Vehicle.kt`, `VehicleService.kt`, `VehicleViewModel.kt`, and `VehicleScreens.kt`, with EN/FR/PT/PT-BR feedback. | Full cars or cars whose driver is not offering seats must not accept new passengers or invitations, and pending requests must be preserved when approval is refused. | Focused iOS `VehicleModelTests`/`VehicleObserverTests`, Android `VehicleModelTest`, Firebase RBAC tests, localization/XML validation, and scoped iOS/Android `git diff --check` passed 2026-07-03. |
| REG-002 | DONE | iOS `RegistrationReviewView` pending/waitlist rows now open the existing `AttendeeProfileDetailView` route for registration reviewers while preserving approve/reject buttons. | Android `RegistrationReviewScreen` now threads `onOpenAttendeeProfile` to the existing `AppRoute.AttendeeProfile` destination from both global and camping-scoped review routes. | Leadership reviewers should be able to inspect attendee detail from registration review, subject to the existing `canViewParticipantProfiles` guard. | Focused iOS `AppPermissionTests`, Android `AppPermissionEvaluatorTest`/`RegistrationReviewViewModelTest`, and scoped iOS/Android `git diff --check` passed 2026-07-03. |
| TRANSPORT-004 | DONE | Church-admin feedback showed a vehicle with driver + 3 passengers rendering as `3 of 5 seats occupied`; iOS now derives seat occupancy as `max(occupiedSeats, driver + approved passengers)` for display, availability, and persistence. | Android now uses the same accounted-occupant derivation in `Vehicle.kt`, vehicle form initialization, vehicle detail seat fractions, and payload writes. | `occupiedSeats` must include the driver. Legacy passenger-only counts must display as driver + approved passengers while preserving extra manually entered occupants. | Focused iOS `VehicleModelTests`/`VehicleObserverTests`, Android `VehicleModelTest`/`VehicleUiStateTest`, Firebase RBAC tests, and scoped Android/iOS `git diff --check` passed 2026-07-03. |
...
| AUD-HOME-001 | VERIFY | Home dashboard | Adaptive hero, personalized/pinned actions, schedule timeline, announcement carousel, family QR pass deck, safe-area handling, current icon-only featured marker, and pull-to-refresh that restarts the dashboard stream without forcing a cold loading state exist. A 1080×2400 light-mode emulator check confirmed the event hero, QR/checklist top-right pin badges, quick actions, schedule, and clean Home reload after the family-query fix. | Complete alternate registration states, a multi-person QR pager account, pin picker, dark mode, small-screen layout, and device pull-to-refresh gesture smoke. |
...
| AUD-SCHEDULE-001 | VERIFY | Schedule/program detail | Current-day selection, vertical timeline, editable day titles, custom program types, reminders, food sync, attendance links, calendar actions, pull-to-refresh that restarts the live schedule observer while preserving cached content on transient failure, iOS-like editor top add action/day-chip rename affordance, all-day overview counts, responsive compact time picker sizing, animated selected-day scroll, and animated chip color/stroke transitions exist. | Verify custom type propagation, reminder replacement, day normalization, selected-day persistence, editor layout against iOS, both calendar handoffs, and device pull-to-refresh gesture smoke. |
